...
def get_study_level_data(study_type):
    """
    Returns a dict, with keys 'train' and 'valid' and respective values as study level data-frames,
    these data-frames contain three columns 'Path', 'Count', 'Label'
    Args:
...
def get_dataloaders(data, batch_size=8, study_level=False):
    ...
    image_datasets = {
        x: ImageDataset(data[x], transform=data_transforms[x]) for x in data_cat
    }
    dataloaders = {
        x: DataLoader(
